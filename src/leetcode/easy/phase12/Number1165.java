...
public class Number1165 {
    ...
    public int calculateTime(String keyboard, String word) {
        ...
        for (int i = 0; i < wordCharArray.length; i++) {
            if (i == 0) {
                sum += map.get(wordCharArray[i]);
            } else {
                sum += Math.abs(map.get(wordCharArray[i]) - map.get(wordCharArray[i - 1]));
            }
        }
    ...
    public static void main(String[] args) {
        String keyword = "pqrstuvwxyzabcdefghijklmno";
        String word = "leetcode";
        System.out.println(new Number1165().calculateTime(keyword, word));
    }
