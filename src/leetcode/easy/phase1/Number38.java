...
public class Number38 {
    ...
    public String countAndSay(int n) {
        ...
        while (--n != 0) {
            ...
            char preChar = preString.charAt(0);
            int preCount = 1;
            for (int i = 1; i < preString.length(); i++) {
                char curChar = preString.charAt(i);
                if (curChar == preChar) {
                    preCount++;
                } else {
                    cur.append(preCount).append(preChar - '0');
                    preChar = curChar;
                    preCount = 1;
                }
            }
            cur.append(preCount).append(preChar);
            preString = cur.toString();
        }
        return preString;
    }
